...
<META HTTP-EQUIV="Expires" CONTENT="-1">
   ...
   <BODY onLoad="setNavigation('stats-general.dsp', '/amsPseudoAdmin/doc/OnlineHelp/wwhelp.htm?context=is_help&topic=IS_Server_SysThreadsScrn');">
      <DIV class="position">
         <TABLE width="100%">

	     %ifvar action equals('kill')%  %invoke amsPseudoAdmin.server.query:killThread%
	     %endinvoke%  %endif%

	     %ifvar action equals('interrupt')%  %invoke amsPseudoAdmin.server.query:interruptThread%
	     %endinvoke%  %endif%

	     %invoke amsPseudoAdmin.server.query:getThreadList%
                        ...
                        <script>writeTD('rowdata');</script>
                            ...
                            %else%
                            	  %ifvar cancancel equals('true')%
                            	  	<TD class="oddcol"><IMG src="images/yellow_check.gif" border=0></TD>
                            	  %else%
                            	  	-
                            	  %endif%	
                            %endif%</TD>
			<script>writeTD('rowdata');</script>
			    %ifvar cankill equals('true')%
			      <TD class="oddcol"><IMG src="icons/delete.gif" border=0></TD>
			    %else%
