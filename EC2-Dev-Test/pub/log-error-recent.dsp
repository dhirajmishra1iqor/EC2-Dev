<HTML>
  <HEAD>
	...
	function checkEverything()
    {
		...
		if ( document.logform.qfromdate != null &&
			...
			{
			//alert(document.getElementById('returnto').value);
				return true;
			}
            ...
            function getTodayDate() {
             ...
             %invoke amsPseudoAdmin.server.query:getSetting%
  ...
  <BODY onLoad="setNavigation('log-error-recent.dsp', '/amsPseudoAdmin/doc/OnlineHelp/wwhelp.htm?context=is_help&topic=IS_Logs_ErrorScrn');">
	...
	%ifvar numlines -notempty%
       %scope param(watt.server.log.maxEntries=numlines)%
            	...
            	%invoke amsPseudoAdmin.server.admin:setSettings%
					%onerror%
					%ifvar errorMessage%
...
<input type="hidden" id="returnto" name="returnto" value='%value ../../returnto%'/>
  ...
  %rename ../../order order -copy%
    ...
    %ifvar order -notempty%
     ...
     %endif%
          <TABLE width=100%>
            <TR>
		%ifvar returnto equals('stats-general')%
		...
		%ifvar returnto equals('stats-general')%
			<TD><LI><A href="stats-general.dsp">Return To Server Statistics</A></LI></TD>
		</TR>
...
%endcomment%
		  ...
		  <TD class="oddrow-l" colspan=1>
			...
			%invoke amsPseudoAdmin.server.auditing:getDestination%
	  ...
	  %rename ../../qfromdate qfromdate -copy%
          ...
          %invoke amsPseudoAdmin.server.query:getPartialLog%
            <TR>
	      <TD colspan=2>
	        <TABLE class="tableView">
                  <TR>
